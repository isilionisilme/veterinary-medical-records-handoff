...
implementation and must be followed consistently by all contributors (human and AI). These standards are normative for implementation and review decisions.
...
- API routes must act as **thin adapters only**, limited to:
...
- Use **snake_case** for Python modules, functions, variables, and database artifacts.
- Use **PascalCase** for classes, enums, and schema models.
- Use **UPPERCASE_SNAKE_CASE** for enum members and constants.
- Use **kebab-case** for URL path segments in REST endpoints.
- Prefix interfaces/ports with a clear role name (e.g., `DocumentRepository`, `TextExtractor`).
- Avoid generic names like `manager`, `helper`, `utils` — name by responsibility.

> **Scope note:** This section covers **code and artifact naming only** (modules, classes, variables, database objects, endpoints).
> Git naming conventions — branch names, commit messages, and pull request titles — are defined in [Way of Working](way-of-working).
